...
static void pr_stm(FILE *out, T_stm stm, int d)
{
  ...
  case T_CJUMP:
    ...
    indent(out,d+1); fprintf(out, "%s,", S_name(stm->u.CJUMP.true_label));
    fprintf(out, "%s", S_name(stm->u.CJUMP.false_label)); fprintf(out, ")");
    break;
  case T_MOVE:
...
static void pr_tree_exp(FILE *out, T_exp exp, int d)
{
  ...
  case T_DOUBLE:
    ...
    break;
  case T_CHAR_:
    indent(out,d); fprintf(out, "CHAR %c", exp->u.CHAR);
    break;
  } /* end of switch */
}
