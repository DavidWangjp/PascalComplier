...
static void pr_stm(FILE *out, T_stm stm, int d)
{
    switch (stm->kind)
    {
        ...
        case T_CJUMP:
            ...
            fprintf(out, ",\n");
            indent(out, d + 1);
            fprintf(out, "%s,", S_name(stm->u.CJUMP.true_label));
            fprintf(out, "%s", S_name(stm->u.CJUMP.false_label));
            fprintf(out, ")");
            break;
        case T_MOVE:
            indent(out, d);
...
static void pr_tree_exp(FILE *out, T_exp exp, int d)
{
    switch (exp->kind)
    {
        ...
        case T_MEM:
            ...
            fprintf(out, "MEM");
            fprintf(out, "(\n");
            pr_tree_exp(out, exp->u.MEM.exp, d + 1);
            fprintf(out, ", %d )", exp->u.MEM.size);
            break;
        case T_TEMP:
            indent(out, d);
        ...
        case T_DOUBLE:
            indent(out, d);
            fprintf(out, "DOUBLE %f", exp->u.DOUBLE);
            break;
        case T_CHAR_:
            indent(out, d);
            fprintf(out, "CHAR %c", exp->u.CHAR);
            break;
    } /* end of switch */
}
