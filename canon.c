...
static void trace(T_stmList list)
{
    ...
    {
        T_stmList true = (T_stmList) S_look(block_env, s->u.CJUMP.true_label);
        T_stmList false = (T_stmList) S_look(block_env, s->u.CJUMP.false_label);
        if (false)
        {
        ...
        { /* convert so that existing label is a false label */
            last->tail->head = T_Cjump(T_notRel(s->u.CJUMP.op), s->u.CJUMP.left,
                                       s->u.CJUMP.right, s->u.CJUMP.false_label,
                                       s->u.CJUMP.true_label);
            last->tail->tail = true;
            trace(true);
        }
        else
        {
            ...
            last->tail->head = T_Cjump(s->u.CJUMP.op, s->u.CJUMP.left,
                                       s->u.CJUMP.right, s->u.CJUMP.true_label, false_label);
            last->tail->tail = T_StmList(T_Label(false_label), getNext());
        }
    }
    else
        assert(0);
